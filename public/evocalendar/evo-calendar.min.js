;(function(factory) {
    ...
    EvoCalendar = (function() {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.selectMonthm = $.proxy(_.selectMonthm, _);
    ...
    EvoCalendar.prototype.initEventListener = function() {
        ...
        _.$elements.innerEl.find('[data-year-val]')
        .off('click.evocalendar')
        .on('click.evocalendar', _.selectYear);

        //set event listener for month change
        _.$elements.innerEl.find('[data-month-val]')
        .off('click.evocalendar')
        .on('click.evocalendar', _.selectMonthm);
    ...
    EvoCalendar.prototype.destroyEventListener = function() {
        ...
        _.$elements.innerEl.find('[data-year-val]')// changed from sidebarEl
        .off('click.evocalendar');

        // remove event listener for eacth month
        _.$elements.innerEl.find('[data-month-val]')// changed from sidebarEl
        .off('click.evocalendar');
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup += '<div class="calendar-inner">'+
                            ...
                            '<button class="icon-button" role="button" data-year-val="prev" title="'+_.initials.dates[_.options.language].previousYearText+'">'+
                                ...
                                '</button>'+
                            '</div>'+
                            '<div class="calendar-monthm">'+
                                '<button class="icon-mbutton" role="button" data-month-val="prev" title="'+_.initials.dates[_.options.language].previousMonthText+'">'+
                                '<span class="chevron-arrow-left"></span>'+
                            '</button>'+
                            '&nbsp;<p></p>&nbsp;'+
                            '<button class="icon-mbutton" role="button" data-month-val="next" title="'+_.initials.dates[_.options.language].nextMonthText+'">'+
                            '<span class="chevron-arrow-right"></span>'+
                        '</button></div>'+
    ...
    EvoCalendar.prototype.buildSidebarYear = function() {
        ...
        // _.$elements.innerEl.find('.calendar-monthm > p').text(_.$active.month);
        var _ = this, markup, title;
        title = _.formatDate(new Date(_.$label.months[_.$active.month] +' 1 '+ _.$active.year), _.options.modifiedFormat, _.options.language);
        _.$elements.innerEl.find('.calendar-monthm > p').text(title);
    ...
    ****************/
    ...
    EvoCalendar.prototype.selectEvent = function(event) {
        ...
        //console.log(eventListEl.find('[data-event-index]'));
        
        // To go to the new link
        //for flight table
            if(_.instanceUid == 0)
            {
                alert(_.options.calendarEvents[index].id);
        ...
        // console.log(_.options.calendarEvents[index].id);
        // alert(_.options.calendarEvents[index].id);
    }
    ...
    EvoCalendar.prototype.selectYear = function(event) {
        ...
        if(yearVal == "prev") {
            --_.$active.year; //changed from year
            _.buttonAction = yearVal;
            _.selectDate('');
            // alert(_.instanceUid);
            ...
            if(_.instanceUid == 0)
            {
                $.ajax({
                    type: "GET",
                    url: "https://localhost:7236/api/dashboard/GetCalendarEventFlight/"+_.$active.year+"/"+_.$active.month,
                    dataType: "json",
                    success: function (result) {
                        if(result!==undefined) {
                            _.addCalendarEvent(result);
                            console.log(result);
                         }
                    }
                });
            ...
            else
            {
                $.ajax({
                    type: "GET",
                    url: "https://localhost:7236/api/dashboard/GetCalendarEventBooking/"+_.$active.year+"/"+_.$active.month,
                    dataType: "json",
                    success: function (result) {
                        if(result!==undefined) {
                            _.addCalendarEvent(result);
            ...
            ++_.$active.year; //changed from month
            _.buttonAction = yearVal;
            _.selectDate('');
            if(_.instanceUid == 0)
            {
            ...
            _.$active.year = yearVal; // changed from month;
        }
    ...
    EvoCalendar.prototype.selectMonthm = function(event)
    {
        var _ = this;
        var el, monthVal;
        _.options.calendarEvents = [];
        ...
        if (typeof event === 'string' || typeof event === 'number') {
            if ((parseInt(event)).toString().length === 4) {
                monthVal = parseInt(event);
            }
        } else {
            el = $(event.target).closest('[data-month-val]');
            monthVal = $(el).data('monthVal');
        }
        if(_.$active.month === 0 && monthVal == "prev")
        {       
            --_.$active.year;
            _.$active.month = 11;
        }
        else if(_.$active.month === 11 && monthVal == "next")
        {       
            ++_.$active.year;
            _.$active.month = 0;
        }
        ...
        {
            if(monthVal == "prev") {
                --_.$active.month; //changed from year
                _.buttonAction = monthVal;
                // alert(_.$active.month);
            if(_.instanceUid == 0)
            {
                $.ajax({
                    type: "GET",
            ...
            else
            {
                ...
                ++_.$active.month; //changed from month
                _.buttonAction = monthVal;
                // alert(_.$active.month);
                ...
                _.$active.month = monthVal.toString(); // changed from month;
            }
        }
        
        //$(_.$elements.calendarEl).trigger("selectMonthm", [_.$active.month])
